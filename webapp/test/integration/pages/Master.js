		...
		onTheMasterPage: {
			...
			actions: {
				...
				iSortTheListOnName: function () {
					return this.iChooseASorter("sortButton", "Sort By <Department>");
				},
				...
				iRememberAnIdOfAnObjectThatsNotInTheList: function () {
					return this.waitFor({
						...
						success: function (oList) {
							var aEntityData = this.getEntitySet("EmployeeInfoCollection");
							var aObjectsNotInTheList = aEntityData.filter(function (oObject) {
								return !oList.getItems().some(function (oListItem) {
									return oListItem.getBindingContext().getProperty("PersonalID") === oObject.PersonalID;
								});
							});
							var sObjectNotInTheListId;

							if (aObjectsNotInTheList.length) {
								sObjectNotInTheListId = aObjectsNotInTheList[0].PersonalID;
							} else {
								// Not enough items all of them are displayed so we take the last one
								sObjectNotInTheListId = aEntityData[aEntityData.length - 1].PersonalID;
							}
							...
							oCurrentItem.bindingPath = "/" + oList.getModel().createKey("EmployeeInfoCollection", {
								PersonalID: sObjectNotInTheListId
							});
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("PersonalID"),
						title: oBindingContext.getProperty("Department")
					};
				}
			},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "text",
							value: "<EmployeeInfoCollection> (0)"
						}),
						success: function () {
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Department");
				},
				...
				theListShouldHaveAllEntries: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						success: function (oList) {
							var aAllEntities = this.getEntitySet("EmployeeInfoCollection");
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							return this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "text",
									value: "<EmployeeInfoCollection> (" + iExpectedLength + ")"
								}),
								success: function () {
